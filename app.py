...
def scrape_ipo_list():
    """
    NSE ke official (unofficial wrapper) package se sirf current (abhi
    open) + upcoming (aane waale) IPOs laata hai. Ye JSON-based hai,
    isliye InvestorGain/Chittorgarh jaisi JS-rendering wali dikkat nahi
    aati.
    NOTE: Pehle yahan listPastIPO() (pichle 1 saal ki listed companies)
    bhi include tha, lekin usse purane (aaj se pehle ke) IPOs bhi list
    mein aa jaate the -- jo nahi chahiye tha. Ab hataya hua hai (P2).
    Jo bhi IPO tool mein ek baar add ho jaaye (aaj, 11-Sep-2026, ya uske
    baad), wo hamesha list mein rahega jab tak manually delete na karo --
    isके liye run_daily_update() mein already logic hai (naye IPO add
    hote hain, purane kabhi auto-remove nahi hote).
    """
    ipos = []
    if not NSE_AVAILABLE:
...
if view_df.empty:
    ...
    # P1 -- st.dataframe use kiya (pehle st.markdown+html table tha, jo
    # resize nahi hota tha). st.dataframe mein columns ko mouse se drag
    # karke chota-bada kar sakte ho (border pe hover karke khींचो), aur
    # use_container_width=True se table apne aap screen/tab ki width ke
    # hisaab se fit ho jaata hai. Header apne aap thoda bold dikhta hai
    # is grid mein; agar aur dark chahiye to column header par sort-icon
    # ke paas hi styling milti hai (st.dataframe ki apni built-in style).
    st.dataframe(
        styled,
        use_container_width=True,
        height=min(600, 45 + 38 * len(view_df)),  # roughly rows ke hisaab se height, max 600px
    )
